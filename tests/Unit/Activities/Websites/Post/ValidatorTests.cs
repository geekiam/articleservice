...
public class ValidatorTests
{
    ...
    {
        var query = new Command { Website = new Website() { Domain = input } };
        var result = _validator.TestValidate(query);
        result.ShouldHaveValidationErrorFor(x => x.Website.Domain);
    }
    ...
    public void Should_Not_Have_validation_error_for_domain_name(string input)
    {
        var query = new Command { Website = new Website() { Domain = input } };
        var result = _validator.TestValidate(query);
        result.ShouldNotHaveValidationErrorFor(x => x.Website.Domain);
    }
}
